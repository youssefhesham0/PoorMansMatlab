...
class MathParser {
    ...
    //    return val;
    //}
    // Why? Because of improper operational order

    // NEW: We force Exponents to be evaluated first!
    double parsePower() {
        double val = parseFactor();
        while (*expr == ' ') expr++;
        if (*expr == '^') {
            expr++;
            val = std::pow(val, parsePower());
        }
        return val;
    }

    // UPDATED: Multiplication and Division happen AFTER Powers
    double parseTerm() {
        double val = parsePower(); // <--- Notice this changed from parseFactor()
        while (true) {
            while (*expr == ' ') expr++;
            if (*expr == '*') { expr++; val *= parsePower(); }
            else if (*expr == '/') { expr++; val /= parsePower(); }
            else break;
        }
        return val;
    }
